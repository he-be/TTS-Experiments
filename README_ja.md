...
- `--inter_segment_silence` でセグメント結合時に挿入する無音のデフォルト値（秒）を指定できます（デフォルト: `1.0`）。UI上のスライダーで0.0〜2.0秒の範囲を0.1秒刻みで調整することもできます。結合前に各セグメントの前後無音（および結合後の全体の前後無音）をトリムしたあと、音声の先頭に固定で0.5秒の無音を追加します。
