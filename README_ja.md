...
**注意**: `--target_duration`を指定しない場合、音素数と言語ごとの音素長に基づいて適切な長さが自動計算されます。この計算は簡易的なものなので、結果が期待通りでない場合は手動で長さを指定してください。
...
#### 文分割とセグメント編集（Advanced Segment Editing）

- UIの `Enable Sentence Segmentation / 文分割を有効化` をONにすると、入力テキストをチャンク（約550文字）および文単位のセグメントに自動分割して生成します。
- **階層構造**: 生成結果は「Chunks (Edit Segments)」アコーディオン内に表示され、チャンクごとの音声・テキストと、さらに詳細なセグメントごとの音声・テキストを確認できます。
- **セグメント再生成**: 各セグメントのテキストを編集して `Regen` ボタンを押すことで、その部分だけを再生成し、全体の音声に反映させることができます。
- **バッチ再生成**: 複数のセグメントのチェックボックス（`Select`）をONにし、`Regenerate Selected Segments (Batch)` ボタンを押すことで、選択したセグメントをまとめて並列に再生成できます。これにより試行錯誤の効率が大幅に向上します。
- `inference_tts_batch_multi_text` による並列推論が有効な場合、これらの処理は高速に行われます。
- 入力テキストが1文のみの場合、セグメンテーションは自動的に無効化され、通常の推論が使用されます。

#### スクリプト生成と評価（Script Generation）

- `Script Generation` タブでは、LLM（Gemini, DeepSeek, Groqなど）を使用してTTS用の台本を自動生成できます。
- **片側漫才スクリプト**: 漫才のボケ・ツッコミのような掛け合い形式のスクリプト生成に特化したプロンプト設定が可能です。
- **マルチモデル生成と評価**: 複数のLLMで候補を生成し、Judgeモデル（Gemini）によって最適な台本を選定する機能も統合されています（コードレベルでの対応）。
